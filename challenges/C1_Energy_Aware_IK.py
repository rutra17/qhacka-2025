import nbformat, types, pathlib, ast

NB_PATH = pathlib.Path(__file__).with_name("C1_Energy_Aware_IK.ipynb")
    ...
    for cell in nb.cells:
        if cell.cell_type == "markdown":
            ...
            if "public tests" in sec:
                continue
            if "baseline" in sec:
                # Keep helper defs (fk, baseline_energy_aware), drop demo runs
...
solve = _ns.get("solve")
if solve is None:
    raise ImportError("`solve` not found in notebook: C1_Energy_Aware_IK.ipynb")